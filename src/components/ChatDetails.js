...
class ChatDetails extends Component {
  ...
  render() {
    let participants = this.props.chat.participants.filter(
    ...
    return (
      <Card
        ...
        style={styles.card}
      >
        <CardHeader className="container">{participants}</CardHeader>
...
const mapStateToProps = (state, ownProps) => {
  ...
  const chat =
    state.chat.chat.find(item => item._id === ownProps.match.params.id) ||
    defaultChat;
